...
import {calculatePrice} from '../src/domain/actions/calculate-price'
import {isHolidayOn} from '../src/domain/actions/isHolidayOn'
    ...
    describe('a day pass', () => {
        ...
        describe('when not a holiday', () => {
            ...
            it('appears to have a discount of 35% on Monday', async () => {
                const result = await calculatePrice(getBasePrice, isHolidayOn(listHolidays))(liftPassType, 21, aRandomMonday)
            ...
            it('Monday for a child is more expensive than for an adult', async () => {
                const result = await calculatePrice(getBasePrice, isHolidayOn(listHolidays))(liftPassType, 14, aRandomMonday)

                const cost = Math.ceil(basePrice * 0.7)
                expect(result).to.eql({cost})
            })
            it('Monday for more than 64y old has bigger reduction', async () => {
                const result = await calculatePrice(getBasePrice, isHolidayOn(listHolidays))(liftPassType, 65, aRandomMonday)
            ...
            it('typically uses base price', async () => {
                const result = await calculatePrice(getBasePrice, isHolidayOn(listHolidays))(liftPassType, 21, notMonday)

                expect(result).to.eql({cost: basePrice})
            })
            it('costs nothing for 5yo', async () => {
                const result = await calculatePrice(getBasePrice, isHolidayOn(listHolidays))(liftPassType, 5, aRandomMonday)
